...
Universal, local-first toolkit for Cursor + MemPalace:
- smart retrieval with anti-stickiness,
- automatic utility scoring (no manual feedback required),
- visual dashboard for route diagnostics.

## Goals

- Work with any project structure (not tied to one codebase).
- Keep telemetry local.
- Make retrieval quality observable and measurable.
- Provide reusable Cursor skill/rule templates.

## Key Features

- `mempalace-smart-search.py`
  - blended ranking (semantic + learned score + recency),
  - diversity routing (MMR + source caps),
  - adaptive anti-stickiness,
  - truth telemetry events with:
    - `telemetry_channel = mcp_auto_utility_v1`
    - `auto_utility` score and band (`high|medium|low`)
- `mempalace-touch.ps1` + `mempalace-route-pulse.py`
  - lightweight tool-touch telemetry (`mcp_tool_touch_v1`)
  - bridge support: plain MCP search can emit smart truth events
- `mempalace-dashboard.py`
  - neural path simulator,
  - anti-stickiness metrics,
  - auto-utility quality overview,
  - help-score health view

## Quick Start (Windows)

1) Configure index sources
...
They are pre-tuned for:
- memory-first workflow,
- automatic utility telemetry,
...
- Utility components:
  ...
  - consistency
  - actionability
  - execution outcome (proxy)
  - stability

## Publishing Notes

- This package is intentionally neutral and reusable.
- Replace local paths and palace config per your environment.
- Do not commit private analytics stores or personal transcripts.
